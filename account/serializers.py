...
class RegisterSerializer(serializers.ModelSerializer):                                                      ####РЕГИСТРАЦИЯ
    ...
    class Meta:
        model = User
        fields = ('email','password','password2','last_name','first_name','username','avatar','phone')
        ###  ПРИ РЕГИСТРАЦИИ USER ДОЛЖЕН ЗАПОЛНЯТЬ ЭТИ ПОЛЯ ||||
...
class LogoutSerializer(serializers.Serializer):
    ...
    def save(self,**kwargs):
        try:
            RefreshToken(self.token).blacklist()    ###  AFTER LOGOUT TOCKEN -> BLACKLIST
        except TokenError:
            self.fail('bad_token')
